...
public void Press(int x, int y) {
    ...
    //set turn buttons visible     
    zug++;
}    
}

public void Turn(int Panel, int Richtung) {
    if (gewonnen((zug % 2)) == false ) {
        if((zug % 2)+1 == 1) {
            Spieler1.drehen((zug % 2),Panel, Richtung);
            gewonnen(1);
        }
        if((zug % 2)+1 == 2) {
            Spieler2.drehen((zug % 2),Panel, Richtung);
            gewonnen(2);
        }
    }
    zug++;
    //set turn buttons invisible
}
...
public boolean schrägLinksOben(int Spieler, int i, int j) {
    ...
    if (Spielfeld.getSpielfeld()[j][j]==Spieler && Spielfeld.getSpielfeld()[j+1][i+1]==Spieler && Spielfeld.getSpielfeld()[j+2][i+2]==Spieler && Spielfeld.getSpielfeld()[j+3][i+3]==Spieler && Spielfeld.getSpielfeld()[j+4][i+4]==Spieler) {
        return true;
    }
    else return false;
}
